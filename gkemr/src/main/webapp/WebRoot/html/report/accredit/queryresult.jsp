<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=gb2312">
<title>报表授权</title>
<link href="../../../css/style.css" rel="stylesheet" type="text/css">
<script type="text/javascript" language="JavaScript1.2" src="../../../js/stm31.js"></script>
<script type="text/javascript" language="JavaScript1.2" src="../../../js/tree.js"></script>
<script type="text/javascript" language="JavaScript1.2" src="../../../js/hide.js"></script>
<script language="javascript">
<!--
function windowOpen(theURL,winName,features,width,hight,scrollbars,top,left) 
{
  var parameter="top="+top+",left="+left+",width="+width+",height="+hight;
  if(scrollbars=="no")
 {parameter+=",scrollbars=no";}
  else
 {parameter+=",scrollbars=yes";}
  window.open(theURL,winName,parameter);
}
//-->
</script>
</head>

<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
<br>
<table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td align="right" class="td_page"><input name="Submit" type="submit" class="buttonface" value="  追加  "></td>
  </tr>
</table>
<table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
  <tr>
    <td align="center" class="td_top"><input type="checkbox" name="checkbox" value="checkbox">    </td>
    <td class="td_top">姓名</td>
    <td class="td_top">性别</td>
    <td class="td_top">单位</td>
    <td class="td_top">部门</td>
    <td class="td_top">职务名称</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">高建正</td>
    <td class="td_02">男</td>
    <td class="td_02">外交部</td>
    <td class="td_02">亚洲司一处</td>
    <td class="td_02">处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">张浩</td>
    <td class="td_02">男</td>
    <td class="td_02">外交部</td>
    <td class="td_02">领事司来华签证处</td>
    <td class="td_02">处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">沈庆</td>
    <td class="td_02">男</td>
    <td class="td_02">外交部</td>
    <td class="td_02">领事出国司签证处</td>
    <td class="td_02">副处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">余武</td>
    <td class="td_02">男</td>
    <td class="td_02">外交部</td>
    <td class="td_02">财务司行政经费处</td>
    <td class="td_02">处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">刘佳</td>
    <td class="td_02">女</td>
    <td class="td_02">外交部</td>
    <td class="td_02">财务司综合处</td>
    <td class="td_02">副处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">沈庆</td>
    <td class="td_02">男</td>
    <td class="td_02">外交部</td>
    <td class="td_02">领事出国司签证处</td>
    <td class="td_02">副处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">丁一</td>
    <td class="td_02">男</td>
    <td class="td_02">外交部</td>
    <td class="td_02">财务司行政经费处</td>
    <td class="td_02">处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">李晓宇</td>
    <td class="td_02">女</td>
    <td class="td_02">外交部</td>
    <td class="td_02">财务司综合处</td>
    <td class="td_02">处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">张立文</td>
    <td class="td_02">男</td>
    <td class="td_02">干部司</td>
    <td class="td_02">考核处</td>
    <td class="td_02">副处长</td>
  </tr>
  <tr>
    <td align="center" class="td_02"><input type="checkbox" name="checkbox" value="checkbox"></td>
    <td class="td_02">吴京</td>
    <td class="td_02">女</td>
    <td class="td_02">财务司</td>
    <td class="td_02">计调处</td>
    <td class="td_02">处长</td>
  </tr>
</table>
<table width="95%"  border="0" cellpadding="0" cellspacing="0" class="table02" align="center">
  <tr>
    <td height="30" align="right"><img src="../../../images/1.gif" width="4" height="5" align="absmiddle"> 首页　 <img src="../../../images/2.gif" width="3" height="5" align="absmiddle"> 上一页　 <img src="../../../images/2-2.gif" width="3" height="5" align="absmiddle"> 下一页　 <img src="../../../images/3.gif" width="4" height="5" align="absmiddle"> 末页　　共 1 页 1 条记录</td>
  </tr>
</table>
</body>
</html>
